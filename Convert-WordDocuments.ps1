...
function Invoke-Word {
    ...
    if ($FileSourcePath -ne '') {
        ...
        Write-Output ('Converting on {0}' -f $FileSourcePath)
        ...
        # try to open the document and save in new format
        try {
            ...
            # Ooops
            Write-Error -Message "Error saving document$($FileSourcePath): Error Exception: $($_.Exception.Message)"
            Exit $ERR_WORDSAVEAS
